...
def approval_action(
    ...
    data: ApprovalAction,
    db: Session = Depends(get_db),
    current: User = Depends(sales_access),
):
    try:
...
def fulfill(
    ...
    if quote.status not in {"APPROVED", "CONFIRMED"}:
        raise HTTPException(400, "Quotation must be approved before fulfillment")
    try:
        overrides = {
            line_id: [item.model_dump() for item in rows]
            for line_id, rows in (data.overrides or {}).items()
        }
...
def invoice_quote(
    ...
    if not quote:
        raise HTTPException(404, "Quotation not found")
    invoices = create_invoices(db, quote)
    audit(db, current.id, "quotation", quote.id, "INVOICES_GENERATED")
    db.commit()
    return invoices
...
def list_invoices(db: Session = Depends(get_db), current: User = Depends(get_current_user)):
    ...
    if current.role == "CUSTOMER":
        ...
        if not profile:
            raise HTTPException(400, "Customer account is not linked to a customer record")
        q = q.filter(Invoice.customer_id == profile.customer_id)
    elif current.role not in {"ADMIN", "SALES", "SALES_MANAGER", "FINANCE"}:
...
def pay_invoice(
    ...
    try:
        payment = record_payment(db, invoice, data.amount, data.reference)
        audit(db, current.id, "invoice", invoice.id, "DEMO_PAYMENT_RECORDED")
        db.commit()
...
def negotiate(
    ...
    profile = db.query(CustomerUser).filter(CustomerUser.user_id == current.id).one_or_none()
    quote = db.get(Quotation, quotation_id)
    if not profile or not quote or quote.customer_id != profile.customer_id:
        raise HTTPException(404, "Quotation not found")
    if data.quotation_line_id:
        line = db.get(QuotationLine, data.quotation_line_id)
        if not line or line.quotation_id != quote.id:
            raise HTTPException(400, "Quotation line does not belong to this quotation")
        if data.requested_discount is not None:
            line.discount_percent = data.requested_discount
    request = NegotiationRequest(quotation_id=quote.id, **data.model_dump())
    db.add(request)
    try:
        ...
        audit(db, current.id, "quotation", quote.id, "NEGOTIATION_SUBMITTED", data.comment)
        db.commit()
        return {
            "negotiation": request,
            "risk": risk,
...
def customer_quote(
    ...
    return {
        ...
        "negotiations": db.query(NegotiationRequest)
        .filter(NegotiationRequest.quotation_id == quote.id)
        .all(),
    }
