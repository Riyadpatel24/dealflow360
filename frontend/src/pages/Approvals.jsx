    ...
    <div className="app-shell approval-page">
      <header className="topbar">
        <div className="brand-lockup">
          ...
          <span className="topbar-subtitle">Sales / Approvals</span>
        </div>
      ...
      <main className="dashboard-container">
        <div className="page-heading">
          <div>
            <div className="eyebrow">GOVERNANCE CENTER</div>
            <h1>Approval Queue</h1>
            <p>Review policy exceptions and move deals through the correct approval chain.</p>
          </div>
        ...
        {loading ? <div className="content-card"><p>Loading approvals...</p></div> : items.length === 0 ? (
          <div className="content-card empty-state">
            ...
            <h3>No active approval requests</h3>
            <p>High-risk quotations will appear here after policy evaluation.</p>
          </div>
        ) : (
                ...
                <section className="content-card approval-card" key={approval.approval_request.id}>
                  <div className="approval-card-head">
                    <div>
                      ...
                      <p>Deal #{quotation.id} · {quotation.currency || "USD"} · {quotation.status.replaceAll("_", " ")}</p>
                    </div>
                  ...
                  <div className="approval-timeline">
                    {approval.steps.map((step, index) => (
                      <div className={`approval-step ${stepTone(step.status)}`} key={step.id}>
                        ...
                        <div className="approval-step-copy">
                          <strong>{step.approver_role.replaceAll("_", " ")}</strong>
                          <span>{step.status.replaceAll("_", " ")}</span>
                        </div>
                        {step.status === "PENDING" && canAct && (
                          <div className="approval-actions">
                            <button className="primary-small-button" disabled={busy === key} onClick={() => decide(quotation.id, step.id, "APPROVE")}>{busy === key ? "Saving..." : "Approve"}</button>
                            <button className="approval-reject-button" disabled={busy === key} onClick={() => decide(quotation.id, step.id, "REJECT")}>Reject</button>
                            <button className="secondary-button" disabled={busy === key} onClick={() => decide(quotation.id, step.id, "RETURN")}>Return</button>
                          </div>
                        )}
